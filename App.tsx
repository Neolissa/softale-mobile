...
function buildCampaignNpcReactionPreset(
  campaign: CampaignId,
  npcBase: string,
  ...
  const flavorByCampaign: Record<CampaignId, string> = {
    forest: "лес и так на нервах",
    romance: "между вами все еще важно не ранить друг друга",
    slytherin: "в круге каждый укол считают",
    boss: "команда читает тон с первой секунды",
    narcissist: "в этом диалоге любая трещина быстро становится петлей",
    "sherlock-gaslighter": "здесь правда держится только на проверяемых фактах",
    "cinderella-advocate": "вежливый тон не должен скрывать давление",
    "healer-empathy": "забота без границ быстро становится выгоранием",
    "partisan-hq": "в штабе цена ошибки всегда коллективная",
    "stop-crane-train-18plus": "здесь срочность опасна без этики решения",
    "first-word-forest": "первое слово часто труднее самого конфликта",
    "dragon-ultimatum": "в этом зале слабый формат сгорает первым",
    "castle-boundaries": "традиции не оправдывают нарушение границ",
    gryffindor_common_room: "гостиная вспыхивает моментально",
    ravenclaw_common_room: "здесь очень быстро проверяют логику на прочность",
    hufflepuff_common_room: "избегание снова тянет разговор в туман",
    "office-icebreaker": "мостик не выдерживает лишнего шума",
    "boundary-keeper": "твои границы здесь проверяют на каждом шаге",
    "serpentine-diplomat": "в кулуарах цену платят за каждую неточность",
    "heart-lines": "между вами важен не только смысл, но и тон",
    "mirror-of-truth": "в разборе важны факты, а не поза",
  };

  const poolsByCampaign: Record<CampaignId, Record<number, [string, string, string]>> = {
    forest: {
      0: ["«С таким ходом мы срываем тропу.»", "«Так лес нас проглотит раньше рассвета.»", "«Это не прорыв, это лишний риск для отряда.»"],
      1: ["«Слышно умно, но шип внутри остался.»", "«Почти рабоче, но укол снова все портит.»", "«Мысль есть, доверия в ней пока нет.»"],
      2: ["«С этим ты просто отдаешь управление.»", "«Молчанием маршрут не держат.»", "«Так ты исчезаешь из роли лидера.»"],
      3: ["«Ок, это уже по делу.»", "«С таким ответом можно идти дальше.»", "«Вот, теперь у отряда появляется опора.»"],
      4: ["«Сильный ход, держи курс.»", "«Это уже лидерство, а не реакция.»", "«Так и собирают группу в шторме.»"],
    },
    romance: {
      0: ["«Сейчас это звучит как удар, не как разговор.»", "«Так мы только ранимся сильнее.»", "«Этим ответом ты обрываешь контакт.»"],
      1: ["«Мягко снаружи, но колко внутри.»", "«Фраза красивая, а шип все равно слышен.»", "«Почти тепло, но снова с уколом.»"],
      2: ["«Тишина сейчас как стена между нами.»", "«Если ты уходишь в молчание, я снова теряю контакт.»", "«С этим ответом близость не двигается.»"],
      3: ["«Да, это уже честный диалог.»", "«Так можно говорить и не разрушать нас.»", "«Ок, в этом есть уважение к нам обоим.»"],
      4: ["«Сильный и живой ответ, я тебя слышу.»", "«Вот так и строят близость без шантажа.»", "«На таком тоне мы правда можем вырасти.»"],
    },
    slytherin: {
      0: ["«Грубо. Для круга слишком прямолинейно.»", "«Такой ход сжигает влияние.»", "«Это не стратегия, это вспышка.»"],
      1: ["«Ровно сказано, но клинок спрятан.»", "«Почти чисто, но укол заметен.»", "«Формально да, по сути снова яд.»"],
      2: ["«С этим голос в круге теряется.»", "«Молчанием ты отдаешь партию.»", "«Так ты сама сдаешь рычаг.»"],
      3: ["«Рабочий шаг, продолжай.»", "«Ок, это уже политически взрослый ход.»", "«Сейчас звучит по правилам круга.»"],
      4: ["«Сильная архитектура влияния.»", "«Вот это ход надолго, не на раунд.»", "«Так и собирают власть без грязи.»"],
    },
    boss: {
      0: ["«С этим ответом отдел снова взорвется.»", "«Такой ход не решает задачу, только накаляет.»", "«Это эскалация, не управление.»"],
      1: ["«Почти конструктивно, но укол остался.»", "«Мысль есть, тон снова режет.»", "«Почти по делу, но контакт ты все равно колешь.»"],
      2: ["«С этим ответом ты отдаешь процесс.»", "«Молчание здесь = потеря управления.»", "«Так ты просто выходишь из игры.»"],
      3: ["«Ок, это уже рабоче.»", "«Вот, сейчас мы правда двигаем задачу.»", "«Такой тон можно нести в команду.»"],
      4: ["«Сильный системный ход.»", "«Это лидерский ответ, и он держит людей.»", "«Вот так меняют культуру, не ломая команду.»"],
    },
    narcissist: {
      0: ["«Этим ты снова ранишь себя.»", "«Так туман только гуще.»", "«Это ответ из боли, не из опоры.»"],
      1: ["«Снаружи спокойно, внутри снова укол.»", "«Фраза ровная, но тебя в ней все еще давят.»", "«Почти граница, но с уступкой себе.»"],
      2: ["«Молчание здесь отдаёт контроль.»", "«С этим ответом петля затягивается.»", "«Ты исчезаешь из собственной позиции.»"],
      3: ["«Да, это уже честно и бережно к себе.»", "«Так можно говорить и не терять опору.»", "«Рабочий ответ, здесь ты остаешься собой.»"],
      4: ["«Сильная граница, тебя не продавить.»", "«Это уже взрослая безопасность.»", "«Так и выходят из качелей без войны.»"],
    },
    gryffindor_common_room: {
      0: ["«Этим ты только поджигаешь зал.»", "«Так команда не соберется, только расколется.»", "«Слишком много удара, слишком мало лидерства.»"],
      1: ["«Звучит ровно, но колкость внутри осталась.»", "«Почти чисто, но подкол все портит.»", "«Смысл есть, но тон снова рвет круг.»"],
      2: ["«Молчишь — отдаешь лидерство.»", "«С этим ответом ты уходишь с поля.»", "«Так команда тебя теряет в ключевой момент.»"],
      3: ["«Ок, это уже командный ход.»", "«Так можно вести, а не просто спорить.»", "«Рабочая рамка, продолжай в этом тоне.»"],
      4: ["«Вот это лидерство без крика.»", "«Сильный ход, круг держится на нем.»", "«Так и рождается уважение, не страх.»"],
    },
    ravenclaw_common_room: {
      0: ["«Импульс есть, доказательности нет.»", "«С таким ответом метод рассыпается.»", "«Это риторика, не аргумент.»"],
      1: ["«Почти чисто, но хвост манипуляции остался.»", "«Умно звучит, но не до конца честно.»", "«Логика есть, прозрачности пока не хватает.»"],
      2: ["«С этим ты отдаешь право на вывод.»", "«Молчание здесь = отказ от аргумента.»", "«Так дискуссия уходит без твоего голоса.»"],
      3: ["«Аргумент рабочий, идем дальше.»", "«Ок, это уже проверяемо.»", "«Сейчас в ответе есть и логика, и рамка.»"],
      4: ["«Сильный аналитический ход.»", "«Вот это уже уровень Когтеврана.»", "«Так и держат точность под давлением.»"],
    },
    hufflepuff_common_room: {
      0: ["«С такой подачей мы снова уходим в обиду.»", "«Это ранит сильнее, чем помогает.»", "«Так правда не услышится, только оттолкнет.»"],
      1: ["«Мягко сказано, но колкость слышна.»", "«Почти бережно, но финальный укол режет.»", "«Смысл верный, форма пока колючая.»"],
      2: ["«Опять тишина, и проблема остается.»", "«С этим молчанием мы снова откладываем важное.»", "«Так разговор глохнет до следующего взрыва.»"],
      3: ["«Да, так можно говорить прямо и не ломать.»", "«Вот это бережная ясность.»", "«Рабочий ответ, в нем есть уважение.»"],
      4: ["«Сильная бережная прямота.»", "«Так и строят теплую, но честную комнату.»", "«Этот тон можно брать в общую норму.»"],
    },
    "office-icebreaker": {
      0: ["«Это эскалация, не управление штормом.»", "«Так мы теряем темп и людей.»", "«С этим ответом мостик снова в пожаре.»"],
      1: ["«Почти конструктивно, но укол ломает контакт.»", "«Мысль рабочая, тон пока режет.»", "«Фраза правильная, подача снова колкая.»"],
      2: ["«Так ты отдаешь руль процесса.»", "«Молчание здесь быстро становится хаосом.»", "«С этим ответом управление уходит.»"],
      3: ["«Ок, рабочий ход.»", "«С этим уже можно двигать задачу.»", "«Вот, сейчас команда слышит структуру.»"],
      4: ["«Сильный ответ, хаос реально гаснет.»", "«Так и держат аварийный ритм без паники.»", "«Это можно брать как новую рабочую норму.»"],
    },
    "boundary-keeper": {
      0: ["«Так ты снова сдаешь свою рамку.»", "«Этим ответом граница трескается.»", "«С таким ходом давление только растет.»"],
      1: ["«Почти граница, но укол в конце мешает.»", "«Сказано ровно, а себя снова чуть сдаешь.»", "«Мысль верная, формулировка пока колючая.»"],
      2: ["«Молчание стирает твою позицию.»", "«С этим ответом рамка расползается.»", "«Так границы снова становятся невидимыми.»"],
      3: ["«Хорошо, это взрослая граница.»", "«Ок, здесь есть и уважение, и ясность.»", "«Рабочий ответ, ты удержала себя.»"],
      4: ["«Сильный ответ, тут тебя уже не продавить.»", "«Вот так и закрепляют границы надолго.»", "«Это уже язык взрослой автономии.»"],
    },
    "serpentine-diplomat": {
      0: ["«Прямой нажим в кулуарах долго не живет.»", "«Такой ход сожжет сделку быстрее, чем подпишет.»", "«Это удар по позиции, не по результату.»"],
      1: ["«Почти дипломатично, но укус слышен.»", "«Снаружи ровно, внутри все еще шипит.»", "«Мысль рабочая, скрытый укол мешает.»"],
      2: ["«С этим ты отдаешь рычаг.»", "«Молчание здесь - это уступка позиции.»", "«Так ты сама уводишь себя с переговорного поля.»"],
      3: ["«Рабочий дипломатический ход.»", "«Ок, это уже язык сделки.»", "«Так можно держать и контакт, и интерес.»"],
      4: ["«Сильный шаг, это игра вдолгую.»", "«Вот так и строят устойчивое влияние.»", "«На таком ходе держится вся конструкция договора.»"],
    },
    "heart-lines": {
      0: ["«Так ты сейчас ранишь, а не сближаешь.»", "«Этим ответом контакт рвется.»", "«Слишком много боли, слишком мало диалога.»"],
      1: ["«Слышу мягкость, но укол остался.»", "«Фраза красивая, а шип все еще там.»", "«Почти бережно, но в конце режет.»"],
      2: ["«Опять тишина вместо близости.»", "«С этим молчанием мы отдаляемся.»", "«Так связь не лечится, а замирает.»"],
      3: ["«Да, это честный и теплый контакт.»", "«Ок, вот так нас можно сохранить.»", "«Рабочая реплика, в ней есть уважение.»"],
      4: ["«Сильный взрослый ответ, на нем можно строить нас.»", "«Так звучит зрелая близость.»", "«Этот ход реально укрепляет связь.»"],
    },
    "sherlock-gaslighter": {
      0: ["«Это версия без опоры на факты.»", "«Так легко потерять реальность в интерпретации.»", "«С этим ходом дело только запутывается.»"],
      1: ["«Почти рабоче, но подмена еще слышна.»", "«Звучит ровно, однако факт по-прежнему размыт.»", "«Мы близко, но в формулировке осталась лазейка.»"],
      2: ["«Молчанием здесь ты отдаешь право на истину.»", "«Без позиции версию за тебя напишут другие.»", "«Так факты утекают из твоих рук.»"],
      3: ["«Ок, это уже проверяемый ход.»", "«С этим можно двигаться по делу.»", "«Хороший шаг: меньше шума, больше данных.»"],
      4: ["«Сильный детективный ответ.»", "«Так и развязывают газлайтинг по узлам.»", "«Очень точный ход, держи эту линию.»"],
    },
    "cinderella-advocate": {
      0: ["«Так ты снова назначаешь виноватой только себя.»", "«Этим ответом давление становится нормой.»", "«Слишком много самоотмены в этой фразе.»"],
      1: ["«Почти бережно, но вина все еще внутри.»", "«Мысль верная, а формулировка еще уступает себя.»", "«Сказано мягко, но граница пока хрупкая.»"],
      2: ["«Тишина здесь делает сценарий только жестче.»", "«С этим молчанием тебя снова не слышат.»", "«Так старая роль закрепляется.»"],
      3: ["«Да, это ясный и уважительный тон.»", "«Рабочая фраза, в ней есть субъектность.»", "«Ок, здесь ты не предаешь себя.»"],
      4: ["«Сильный ход: и тепло, и граница на месте.»", "«Вот так звучит взрослая самоценность.»", "«Отличный ответ, сценарий разворачивается в твою пользу.»"],
    },
    "healer-empathy": {
      0: ["«Так ты опять идешь в самопожертвование.»", "«Этим ходом ресурс быстро сгорит.»", "«Слишком тяжело для тебя и бесполезно для системы.»"],
      1: ["«Почти заботливо, но граница еще дрожит.»", "«Тепло есть, опоры пока недостаточно.»", "«Смысл хороший, формулировку нужно усилить.»"],
      2: ["«Молчание здесь звучит как согласие на перегруз.»", "«Если не обозначить пределы, тебя снова продавят.»", "«Так ты исчезаешь из собственного решения.»"],
      3: ["«Ок, это здоровая эмпатия.»", "«Рабочий баланс: участие без самопотери.»", "«Да, с таким ответом можно помогать устойчиво.»"],
      4: ["«Сильный ход Лекаря с границами.»", "«Вот это зрелая поддержка без выгорания.»", "«Отлично: и человечески, и безопасно для тебя.»"],
    },
    "partisan-hq": {
      0: ["«Так штаб трещит по швам.»", "«С этим ходом мы кормим панику.»", "«Слишком рискованно для общего контура.»"],
      1: ["«Почти рабоче, но укол еще внутри.»", "«Мысль есть, доверие пока хрупкое.»", "«Звучит ровно, но формат еще шатается.»"],
      2: ["«Молчание здесь = потеря координации.»", "«С этим ответом ты отдаешь инициативу.»", "«Так команда остается без опоры.»"],
      3: ["«Ок, это рабочий командный ход.»", "«Так штаб можно удержать в контакте.»", "«Хорошо, здесь есть зрелая рамка.»"],
      4: ["«Сильный стратегический ответ.»", "«Так и собирают сопротивление под давлением.»", "«Отличный ход, это держит систему.»"],
    },
    "stop-crane-train-18plus": {
      0: ["«Это путь в дорогую ошибку.»", "«Так скорость победит безопасность.»", "«С этим выбором цена слишком высока.»"],
      1: ["«Почти, но этика еще размыта.»", "«Мысль есть, критерии пока не дожаты.»", "«Сказано ровно, но риски не закрыты.»"],
      2: ["«Молчание здесь опаснее любого слова.»", "«Так решение примут за тебя.»", "«С этим ответом ты теряешь контроль узла.»"],
      3: ["«Ок, это зрелый шаг под давлением.»", "«Такой выбор держит и людей, и процесс.»", "«Хорошо, здесь есть ответственность.»"],
      4: ["«Сильный этический ход.»", "«Так и проходят вагонетку по-взрослому.»", "«Отлично: ясность, смелость и последствия учтены.»"],
    },
    "first-word-forest": {
      0: ["«Так ты снова прячешься от диалога.»", "«Этим ходом начало разговора срывается.»", "«Слишком много защиты, мало ясности.»"],
      1: ["«Почти честно, но есть самоукол.»", "«Тон живой, формулировка еще дрожит.»", "«Мы близко, но граница не до конца ясна.»"],
      2: ["«С этим молчанием все остается как было.»", "«Так первый шаг снова откладывается.»", "«Ты исчезаешь из собственной позиции.»"],
      3: ["«Да, это рабочее первое слово.»", "«Так можно входить в сложный контакт.»", "«Хорошо, в этом есть опора.»"],
      4: ["«Сильный мягкий старт.»", "«Вот так и рождается зрелый диалог.»", "«Отличный ход, ты удержала и себя, и контакт.»"],
    },
    "dragon-ultimatum": {
      0: ["«С этим ходом ты отдаешь поле ультиматуму.»", "«Такой ответ кормит давление силой.»", "«Это не переговоры, это уступка под угрозой.»"],
      1: ["«Почти твердо, но лазейка еще есть.»", "«Мысль верная, рамка пока мягкая.»", "«Звучит сильно, но структуру нужно укрепить.»"],
      2: ["«Молчание здесь считывают как капитуляцию.»", "«С этим ответом правило зададут за тебя.»", "«Так ты отдаешь контур будущей сделки.»"],
      3: ["«Ок, это рабочий переговорный шаг.»", "«Так можно удержать достоинство и процесс.»", "«Хорошо, ты ведешь конфликт в рамку.»"],
      4: ["«Сильный огнеупорный ответ.»", "«Так и гасят ультиматум без войны.»", "«Очень мощный ход, структура сильнее угрозы.»"],
    },
    "castle-boundaries": {
      0: ["«Так старая роль снова побеждает.»", "«Этим ответом граница тает.»", "«Слишком много удобства, слишком мало тебя.»"],
      1: ["«Почти бережно, но вина еще вшита.»", "«Фраза ровная, а позиция пока хрупкая.»", "«Мысль есть, границу стоит усилить.»"],
      2: ["«Молчание здесь закрепляет давление.»", "«С этим ходом тебя снова не слышно.»", "«Так ты уходишь из собственного выбора.»"],
      3: ["«Да, это ясная взрослая граница.»", "«Ок, тут есть и уважение, и позиция.»", "«Хороший ход, ты не отменяешь себя.»"],
      4: ["«Сильный ответ с достоинством.»", "«Так и строят новый порядок в доме.»", "«Отлично: мягко по тону, твердо по сути.»"],
    },
    "mirror-of-truth": {
      0: ["«Эмоция сейчас громче метода.»", "«С таким входом верификация падает.»", "«Это реакция, не аналитика.»"],
      1: ["«Почти чисто, но манипулятивный хвост остался.»", "«Смысл верный, подача пока не до конца честная.»", "«Фраза рабочая, но все еще с подменой.»"],
      2: ["«С этим ты отдаешь право на вывод.»", "«Молчание здесь быстро становится искажением.»", "«Так анализ уходит из твоих рук.»"],
      3: ["«Рабочий аналитический шаг.»", "«Ок, метод выдержан, продолжаем.»", "«Это уже разговор по данным.»"],
      4: ["«Сильная и точная позиция.»", "«Вот это взрослый уровень разбора.»", "«Так и держат правду под давлением.»"],
    },
  };

  const quarterUnique = (_index: number): Record<number, string> => {
    const shortHint = toShortPlayerLine(node.hint, 10);
    return {
      0: `${npcBase} резко осекает: «Такой ответ рвет сцену «${shortHint}» и отбрасывает нас назад».`,
      1: `${npcBase} ловит полутона: «Мысль почти верная, но укол в конце снова все портит».`,
      2: `${npcBase} мрачнеет: «Молчание выглядит как сдача, и мы возвращаемся в старый круг».`,
      3: `${npcBase} кивает: «Это рабочий поворот: и контакт держится, и задача двигается».`,
      4: `${npcBase} заметно теплеет: «Это сильный шаг - он двигает сюжет и показывает твой рост».`,
    };
  };

  // Каждый 4-й вопрос получает уникальные реакции по сцене.
  if ((idx + 1) % 4 === 0) {
    return quarterUnique(idx);
  }

  const pool = poolsByCampaign[campaign];
  const variantIdx = idx % 3;
  return {
    0: `${npcBase} напрягается: ${pool[0][variantIdx]}`,
    1: `${npcBase} прищуривается: ${pool[1][variantIdx]}`,
    2: `${npcBase} перехватывает инициативу: ${pool[2][variantIdx]}`,
    3: `${npcBase} чуть смягчает тон: ${pool[3][variantIdx]}`,
    4: `${npcBase} делает паузу и признает ход: ${pool[4][variantIdx]}`,
  };
}
...
function buildLitRpgStepOptions(campaign: CampaignId, node: QuestNarrativeNode, idx: number, stageIdx: number) {
  ...
  if (editorialStep) {
    ...
    const optionNpcReactionByIndex = buildCampaignNpcReactionPreset(campaign, npcBase, node, idx, stageIdx);
...
function buildCampaignInstructionLead(campaign: CampaignId) {
  const leads: Record<CampaignId, string> = {
    ...
    "sherlock-gaslighter": "В этом деле побеждает факт, а не уверенный тон.",
    "cinderella-advocate": "Здесь важно остаться доброй, не становясь удобной.",
    "healer-empathy": "Помогай так, чтобы после помощи у тебя оставалась опора.",
    "partisan-hq": "Команду спасает не крик, а ясный порядок.",
    "stop-crane-train-18plus": "Сложный выбор начинается с этики, а не с паники.",
    "first-word-forest": "Первое честное слово уже меняет траекторию разговора.",
    "dragon-ultimatum": "На ультиматум отвечают структурой, а не страхом.",
    "castle-boundaries": "Теплый тон и твердая граница могут жить вместе.",
...
function buildCampaignSceneAccent(campaign: CampaignId, idx: number) {
  const accents: Record<CampaignId, string[]> = {
    ...
    "sherlock-gaslighter": ["Спокойная ложь выглядит убедительно, пока нет сверки фактов.", "Хронология - твоя защита от подмены реальности."],
    "cinderella-advocate": ["Вежливость не равна согласию терпеть давление.", "Твоя граница может звучать мягко и твердо одновременно."],
    "healer-empathy": ["Сочувствие работает только там, где есть ресурс.", "Граница не отменяет доброту, она делает ее устойчивой."],
    "partisan-hq": ["Доверие команды строится на прозрачных ролях.", "Паника гасится ясностью формата, а не давлением."],
    "stop-crane-train-18plus": ["Скорость без критериев безопасности становится ловушкой.", "Этический выбор требует ясной ответственности за последствия."],
    "first-word-forest": ["Молчание защищает недолго, ясность работает дольше.", "Первый шаг в диалог начинается с короткой честной фразы."],
    "dragon-ultimatum": ["Сила позиции проверяется под угрозой.", "Устойчивый договор сильнее громкого ультиматума."],
    "castle-boundaries": ["Традиции не отменяют право на границы.", "Самоуважение звучит спокойно и без оправданий."],
...
function buildCampaignHintLead(campaign: CampaignId) {
  const leads: Record<CampaignId, string> = {
    forest: "Подумай так:",
    romance: "Подумай так:",
    slytherin: "Подумай так:",
    boss: "Подумай так:",
    narcissist: "Подумай так:",
    "sherlock-gaslighter": "Подумай так:",
    "cinderella-advocate": "Подумай так:",
    "healer-empathy": "Подумай так:",
    "partisan-hq": "Подумай так:",
    "stop-crane-train-18plus": "Подумай так:",
    "first-word-forest": "Подумай так:",
    "dragon-ultimatum": "Подумай так:",
    "castle-boundaries": "Подумай так:",
    gryffindor_common_room: "Подумай так:",
    ravenclaw_common_room: "Подумай так:",
    hufflepuff_common_room: "Подумай так:",
    "office-icebreaker": "Подумай так:",
    "boundary-keeper": "Подумай так:",
    "serpentine-diplomat": "Подумай так:",
    "heart-lines": "Подумай так:",
    "mirror-of-truth": "Подумай так:",
  };
  return leads[campaign];
}
  ...
  const steps = nodes.map((node, idx) => {
    ...
    return {
      ...
      hint: hasEditorialStep
        ? `${buildCampaignHintLead(campaign)} ${node.hint} ${campaignStoryArc[campaign].finale}`
        : `${buildCampaignHintLead(campaign)} Этот шаг помечен как needsEditorialFill: добавьте ручные варианты ответов в библиотеку.`,
