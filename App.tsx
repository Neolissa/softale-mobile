...
function buildCampaignNpcReactionPreset(
  ...
  const quarterUnique = (index: number): Record<number, string> => {
    const stageLabel = `этапе ${stageIdx + 1}, вопросе ${index + 1}`;
    const shortHint = toShortPlayerLine(node.hint, 10);
    return {
      0: `${npcBase} резко осекает: «На ${stageLabel} такой ответ рвет сцену «${shortHint}» и отбрасывает нас назад».`,
      1: `${npcBase} ловит полутона: «На ${stageLabel} мысль почти верная, но укол в конце снова все портит».`,
      2: `${npcBase} мрачнеет: «На ${stageLabel} молчание выглядит как сдача, и мы возвращаемся в старый круг».`,
      3: `${npcBase} кивает: «Для ${stageLabel} это рабочий поворот: и контакт держится, и задача двигается».`,
      4: `${npcBase} заметно теплеет: «На ${stageLabel} это сильный шаг - он двигает сюжет и показывает твой рост».`,
    };
  };
...
export default function App() {
  ...
  const passStep = (npcReaction?: string, tactic?: BranchId, tier: "angel" | "good" | "neutral" = "good") => {
    ...
    if (isLastStep) {
      ...
      const praiseLead = tier === "angel" ? "Ангельский ход!" : tier === "neutral" ? "Нейтральный ход." : "Классный ход!";
      setStepMessage(`Квест завершен! За шаг: ${rewardText}.`);
      setLastStepPraise(`${npcReaction ? `${npcReaction} ` : ""}${praiseLead} За предыдущее задание: ${rewardText}.`);
      finishForestQuest();
      return;
    }
    ...
    if (isStageBoundary) {
      ...
      setLastStepPraise(`${npcReaction ? `${npcReaction} ` : ""}Этап ${currentStage + 1} закрыт. За шаг: ${stageRewardText}.`);
    ...
    const rewardText = reward > 0 ? `+${reward} XP` : "без изменений XP";
    const praiseLead = tier === "angel" ? "Ангельский ход!" : tier === "neutral" ? "Нейтральный ход." : "Ход засчитан.";
    setLastStepPraise(`${npcReaction ? `${npcReaction} ` : ""}${praiseLead} За шаг: ${rewardText}.`);
    setStepMessage(`${npcReaction ? `${npcReaction} ` : ""}Ход принят. За шаг: ${rewardText}. Переходим дальше.`);
  ...
  const finalizeIncorrectStepAsAccepted = (
    ...
    const isStageBoundary = !isLastStep && nextStage !== currentStage;
    if (isLastStep) {
      setStepMessage(`${npcReaction ? `${npcReaction} ` : ""}${tier === "superbad" ? "Суперплохой ответ" : "Плохой ответ"}: -${penalty} XP. Квест завершен.`);
    ...
    if (isStageBoundary) {
      ...
      setStepMessage(`${npcReaction ? `${npcReaction} ` : ""}Этап завершен со штрафом (-${penalty} XP). Открыт этап ${nextStage + 1}.`);
      setForestStepIndex(nextStepIndex);
      setForestStarted(false);
      resetStepUi();
      trackAnalyticsEvent("stage_complete", {
    ...
    setLastStepPraise(`${npcReaction ? `${npcReaction} ` : ""}Шаг засчитан со штрафом. Разбор доступен в профиле.`);
    setStepMessage(`${npcReaction ? `${npcReaction} ` : ""}${tier === "superbad" ? "Суперплохой ответ" : "Плохой ответ"}: -${penalty} XP. Идем дальше.`);
    setForestStepIndex(nextStepIndex);
    resetStepUi();
  };
