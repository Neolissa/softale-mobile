...
## 5.5) Контент и реакции NPC

- `npm run content:audit` — без запрещённых маркеров и критичных дубликатов опций.
- `npm run build:npc-reactions` — актуальный `content/npc-reactions/all.json` после правок сценария или пулов реакций.
